...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if opts.rgt:
            data['rgt'] = read_array(self.dir_data + '/' + idx + '_rgt.bin', (1, self.n1, self.n2))
            target['rgt'] = read_array(self.dir_target + '/' + idx + '_rgt.bin', (1, self.n1, self.n2))

        if opts.dhr:
            data['dhr'] = read_array(self.dir_data + '/' + idx + '_dhr.bin', (1, self.n1, self.n2))
...
class refinenet(pl.LightningModule):
    def __init__(self, lr: float = 1.0e-4):
        ...
        self.encoder1 = resu1(4 * 4, self.l1)
    ...
    def forward(self, x):
        ...
        out_encoder1 = self.encoder1(torch.cat((i1, i2, i3, i4), dim=1))
        ...
        out = {}
        if opts.rgt:
            out_rgt = self.decoder_rgt(x['rgt'], out_encoder1, out_encoder2, out_encoder3)
            out['rgt'] = out_rgt

        if opts.dhr:
            out_dhr = self.decoder_dhr(x['dhr'], out_encoder1, out_encoder2, out_encoder3)
            out['dhr'] = out_dhr
...
if __name__ == '__main__':
            ...
            for (input, target) in valid_loader:
                ...
                for i in range(0, input['rgt'].shape[0]):
                    ...
                    if opts.rgt:
                        ...
                        rgt = np.zeros((opts.n1, opts.n2))
                    if opts.dhr:
                        dhr = target['dhr'][i].squeeze().data.cpu().numpy()
                    else:
                        dhr = np.zeros((opts.n1, opts.n2))
                    if opts.fault:
                        fsem = target['fsem'][i].squeeze().data.cpu().numpy()
                        fdip = target['fdip'][i].squeeze().data.cpu().numpy()
                    else:
                        fsem = np.zeros((opts.n1, opts.n2))
                        fdip = np.zeros((opts.n1, opts.n2))

                    ax[0][0].imshow(rgt, cmap='jet'), ax[0][0].set_title('rgt')
                    ax[0][1].imshow(dhr, cmap='gray'), ax[0][1].set_title('dhr')
                    ax[0][2].imshow(fsem, cmap='viridis'), ax[0][2].set_title('fsem')
                    ax[0][3].imshow(fdip, cmap='jet'), ax[0][3].set_title('fdip')

                    if opts.rgt:
                        rgt = input['rgt'][i].squeeze().data.cpu().numpy()
                    else:
                        rgt = np.zeros((opts.n1, opts.n2))
                    if opts.dhr:
                        dhr = input['dhr'][i].squeeze().data.cpu().numpy()
                    else:
                        dhr = np.zeros((opts.n1, opts.n2))
                    if opts.fault:
                        fsem = input['fsem'][i].squeeze().data.cpu().numpy()
                        fdip = input['fdip'][i].squeeze().data.cpu().numpy()
                    else:
                        fsem = np.zeros((opts.n1, opts.n2))
                        fdip = np.zeros((opts.n1, opts.n2))

                    ax[1][0].imshow(rgt, cmap='jet'), ax[1][0].set_title('rgt')
                    ax[1][1].imshow(dhr, cmap='gray'), ax[1][1].set_title('dhr')
                    ax[1][2].imshow(fsem, cmap='viridis'), ax[1][2].set_title('fsem')
                    ax[1][3].imshow(fdip, cmap='jet'), ax[1][3].set_title('fdip')
                    ...
                    ax[2][0].imshow(rgt, cmap='jet'), ax[2][0].set_title('rgt')
                    ax[2][1].imshow(dhr, cmap='gray'), ax[2][1].set_title('dhr')
                    ax[2][2].imshow(fsem, cmap='viridis'), ax[2][2].set_title('fsem')
                    ax[2][3].imshow(fdip, cmap='jet'), ax[2][3].set_title('fdip')
    ...
    if opts.input is not None:
        ...
        img = {}
        if opts.rgt:
            img['rgt'] = read_array(opts.input + '.rgt', (1, 1, n1, n2)).to(device)
        if opts.dhr:
            img['dhr'] = read_array(opts.input + '.dhr', (1, 1, n1, n2)).to(device)
        if opts.fault:
            img['fsem'] = read_array(opts.input + '.fsem', (1, 1, n1, n2)).to(device)
            img['fdip'] = read_array(opts.input + '.fdip', (1, 1, n1, n2)).to(device)
