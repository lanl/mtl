...
# Multi-task refinement requires all inputs
assert opts.rgt and opts.fault and opts.dhr
...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if opts.rgt:
            data['rgt'] = read_array(self.dir_data + '/' + idx + '_rgt.bin', (1, self.n1, self.n2, self.n3))
            target['rgt'] = read_array(self.dir_target + '/' + idx + '_rgt.bin', (1, self.n1, self.n2, self.n3))

        if opts.dhr:
            data['dhr'] = read_array(self.dir_data + '/' + idx + '_dhr.bin', (1, self.n1, self.n2, self.n3))
            target['dhr'] = read_array(self.dir_target + '/' + idx + '_dhr.bin', (1, self.n1, self.n2, self.n3))

        if opts.fault:
            data['fsem'] = read_array(self.dir_data + '/' + idx + '_fsem.bin', (1, self.n1, self.n2, self.n3))
            data['fdip'] = read_array(self.dir_data + '/' + idx + '_fdip.bin', (1, self.n1, self.n2, self.n3))
            data['fstrike'] = read_array(self.dir_data + '/' + idx + '_fstrike.bin', (1, self.n1, self.n2, self.n3))
...
def custom_loss(y_pred, y_true):
    ...
    # sum
    loss = loss_rgt + loss_dhr + loss_fault_semantic + loss_fault_dip + loss_fault_strike

    return loss, loss_rgt, loss_dhr, loss_fault_semantic, loss_fault_dip, loss_fault_strike
...
class refinenet(pl.LightningModule):
    def __init__(self, lr: float = 1.0e-4):
        ...
        self.input4 = nn.Sequential(conv(self.in_ch, 4), conv(4, 4))
        self.input5 = nn.Sequential(conv(self.in_ch, 4), conv(4, 4))
        self.encoder1 = resu1(4 * 5, self.l1)
    ...
    def forward(self, x):
        ...
        i1 = self.input1(x['rgt'])
        i2 = self.input2(x['dhr'])
        i3 = self.input3(x['fsem'])
        i4 = self.input4(x['fdip'])
        i5 = self.input5(x['fstrike'])

        out_encoder1 = self.encoder1(torch.cat((i1, i2, i3, i4, i5), dim=1))
        ...
        out = {}
        if opts.rgt:
            out['rgt'] = self.decoder_rgt(x['rgt'], out_encoder1, out_encoder2, out_encoder3)

        if opts.dhr:
            out['dhr'] = self.decoder_dhr(x['dhr'], out_encoder1, out_encoder2, out_encoder3)

        if opts.fault:
            out_fault = self.decoder_fault(x['fsem'], out_encoder1, out_encoder2, out_encoder3)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, loss_rgt, loss_dhr, loss_fault_semantic, loss_fault_dip, loss_fault_strike = custom_loss(y_pred, y_true)
...
if __name__ == '__main__':
    ...
    if opts.input is None and opts.check is not None:
        ...
        with tqdm(total=len(v), desc='', unit='image') as pbar:
            ...
            for (input, target) in valid_loader:
                ...
                input['dhr'] = input['dhr'].to(device)
                input['rgt'] = input['rgt'].to(device)
                input['fsem'] = input['fsem'].to(device)
                input['fdip'] = input['fdip'].to(device)
                input['fstrike'] = input['fstrike'].to(device)

                with torch.no_grad():
                    predict = net(input)

                for i in range(0, input['rgt'].shape[0]):

                    ir = (l - 1) * opts.batch_valid + i

                    if opts.rgt:
                        rgt = predict['rgt'][i].squeeze().data.cpu().numpy()
                        write_array(rgt, opts.dir_output + '/predict_' + str(ir) + '_rgt.bin')
                    if opts.dhr:
                        dhr = predict['dhr'][i].squeeze().data.cpu().numpy()
                        write_array(dhr, opts.dir_output + '/predict_' + str(ir) + '_dhr.bin')
                    if opts.fault:
                        fsem = predict['fsem'][i].squeeze().data.cpu().numpy()
                        fdip = predict['fdip'][i].squeeze().data.cpu().numpy()
                        fstrike = predict['fstrike'][i].squeeze().data.cpu().numpy()
    ...
    if opts.input is not None:
        ...
        img = {}
        if opts.rgt:
            img['rgt'] = read_array(opts.input + '.rgt', (1, 1, n1, n2, n3)).to(device)
        if opts.dhr:
            img['dhr'] = read_array(opts.input + '.dhr', (1, 1, n1, n2, n3)).to(device)
        if opts.fault:
            img['fsem'] = read_array(opts.input + '.fsem', (1, 1, n1, n2, n3)).to(device)
            img['fdip'] = read_array(opts.input + '.fdip', (1, 1, n1, n2, n3)).to(device)
            img['fstrike'] = read_array(opts.input + '.fstrike', (1, 1, n1, n2, n3)).to(device)
        ...
        if opts.fault:
            ...
            fstrike = get_numpy(predict['fstrike'])
            write_array(fsem, opts.output + '.fsem')
            write_array(fdip, opts.output + '.fdip')
            write_array(fstrike, opts.output + '.fstrike')
